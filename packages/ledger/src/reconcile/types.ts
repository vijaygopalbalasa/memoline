...
export type LedgerEntry = {
   ...
   * from on-chain memo data on import, copied from the row on the payout side. A memo written by
   * another app as plain text gives its cleaned text (see `memoReference`). Null when the memo is
   * absent, binary, or a Memoline memo with no ref. This is the "line in your books" the product
   * promises. */
