...
| 2 | **Domain** (e.g. `memoline.io`) | The canonical production URL. It becomes `NEXT_PUBLIC_APP_URL` and the SIWE sign-in domain. Attach it to the Vercel project once created. |
...
| `ALLOW_PLACEHOLDER_WALLETCONNECT` | — | `1` | Build, **only** for a secret-less CI build (`next build` with no real `WALLETCONNECT_PROJECT_ID`). Never set this on the real deployment, because WalletConnect and mobile wallets then silently do not work. |
...
3. **Framework Preset**: Next.js (auto-detected). Leave **Build Command** on its default. Vercel runs the
   ...
   **If you ever override the Build Command manually, keep the `--webpack` flag.** `@memoline/ledger`
   ...
   file; Turbopack cannot follow that mapping ([next.js#82945](https://github.com/vercel/next.js/issues/82945)), so plain `next build` (Turbopack, the Next 16 default) fails on it. `apps/web/next.config.ts` documents this.
...
5. Add every environment variable from the table above under **Settings → Environment Variables**:
   ...
   Run `vercel deploy` from the **repository root**, which is the directory linked to the
   `memoline` project. Running it inside `apps/web` links that folder to a new, unrelated project
   and the build fails there (no lockfile).
6. Deploy.
